...
if [ -f /etc/uuid ];then 
	if [ x"$link_cmd" == x"wget" ];then
		if [ `whoami` != "root" ];then
			if [ `whoami` = "ubuntu" ];then
				...
				sudo wget -q  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
				sudo_operation
			else
			...
			wget -q  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
			operation
		fi
	else
		if [ `whoami` != "root" ];then
			if [ `whoami` = "ubuntu" ];then
				sudo echo "[1]Downloading Tool..."
				sudo curl -O  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
				sudo_operation
			else
				sudo echo " only root can run me"
				sudo exit 1
			...
			echo "[1]Downloading Tool..."
			curl -O  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
			operation
		fi
	fi
else
	...
	if [ x"$link_cmd" == x"wget" ];then
		if [ `whoami` != "root" ];then
			if [ `whoami` = "ubuntu" ];then
				sudo echo "[1]Downloading Tool..."
				sudo $netns_prefix wget -q  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
				sudo_operation
			else
				sudo echo " only root can run me"
				sudo exit 1
			...
			echo "[1]Downloading Tool..."
			$netns_prefix wget -q  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
			operation
		fi
	else
		if [ `whoami` != "root" ];then
			if [ `whoami` = "ubuntu" ];then
				sudo echo "[1]Downloading Tool..."
				sudo $netns_prefix curl -O  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
				sudo_operation
			else
				sudo echo " only root can run me"
				sudo exit 1
			...
			echo "[1]Downloading Tool..."
			$netns_prefix curl -O  http://mirrors.tencentyun.com/install/platform_ops/qcloud/log_collect.zip >> /dev/null
			operation
		fi
	fi
fi
