...
crash_info() {
        ...
        if  dpkg -s kexec-tools >/dev/null 2>&1 ;then
           ...
           DUMPDIR=`grep ^KDUMP_COREDIR /etc/default/kdump-tools | cut -d '"' -f2 | tail -1`
           ...
           if [ -d $DUMPDIR ]; then
                ...
                crash_dir=`ls -lt $DUMPDIR|grep "^d"|awk '{print$9}' | awk 'NR==1{print}'`
...
os_ver=$(cat /etc/lsb-release |grep DISTRIB_DESCRIPTION |cut -d '"' -f2)
