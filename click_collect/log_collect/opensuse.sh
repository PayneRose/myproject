...
net_info() {
	...
	IPADDRS=`ifconfig eth0 | grep inet | awk -F' ' '{print$2}' | awk -F':' '{print$2}'`
...
crash_info() {
        ...
        if rpm -q kexec-tools >/dev/null 2>&1 ;then
                ...
                KDUMP_CONFIG_FILE="/etc/sysconfig/kdump"
		a=`grep ^KDUMP_SAVEDIR /etc/sysconfig/kdump | awk -F"\"" '{print$2}'`
                DUMPDIR=${a#*//}
...
messages_file() {
        ...
        log_item $OF  "tail_messages" "tail -n 50000 /var/log/messages"
...
##main
if [[ ! -f /etc/os-release &&  X`cat /etc/os-release | grep -i suse` = X ]] ;then
        echo "not opensuse system,exit."
        exit 1
fi
